...
use super::{ProfileSettings, SandboxEntry};
...
const HOST_GATEWAY: &str = "host.lima.internal";
    ...
    if instance_exists(id).await {
        ...
        if ui::is_verbose() {
            ...
            let output = run_limactl(
                vec!["--tty=false".into(), "start".into(), id.to_string()],
                240,
                &format!("start {}", id),
            )
            .await?;
...
impl LimaBackend {
    pub async fn resolve_settings(
        ...
        let manifest = super::shared::load_profile_manifest(profile_name).await?;
    ...
    pub async fn start(
        ...
        let (id, project_root, _) = resolve_workspace_context().await?;
        if id == "tnk-config" {
            return Err(color_eyre::eyre::eyre!(
                "sandbox start is only available inside a project directory"
            ));
        }
        let _lock =
        ...
        ensure_instance_running(&id, &project_root, settings).await?;

        if needs_provision {
            let active_model =
                crate::sandbox::shared::resolve_active_model(&cfg.engine_runtime, &cfg.model)?;
    ...
    pub async fn shell(
        ...
        let (id, project_root, workdir) = resolve_workspace_context().await?;
        if id == "tnk-config" {
            return Err(color_eyre::eyre::eyre!(
        ...
        let runtime_envs = Self::runtime_env(cfg, &active_model);
    ...
    pub async fn delete(id: &str, force: bool) -> Result<(), color_eyre::Report> {
        delete_lima_instance(id, force).await
    }

    pub async fn ls() -> Result<Vec<SandboxEntry>, color_eyre::Report> {
        list_lima_instances().await
    }

    pub async fn exists(id: &str) -> Result<bool, color_eyre::Report> {
    ...
    pub fn runtime_env(
        ...
        let inference_url = format!("http://{}:{}/v1", HOST_GATEWAY, cfg.server_port);

        vec![
            ("TNK_INFERENCE_URL".to_string(), inference_url),
            ("TNK_MODEL_NAME".to_string(), model_name.to_string()),
            (
                "TNK_ENGINE_RUNTIME".to_string(),
                cfg.engine_runtime.to_string(),
            ),
        ]
    }
}
...
pub async fn resolve_workspace_context() -> Result<(String, PathBuf, PathBuf), color_eyre::Report> {
    ...
    let resolved = crate::config::ResolvedConfig::resolve(&crate::config::load().await?)?;
...
async fn stop_lima_instance() -> Result<(), color_eyre::Report> {
    ...
    let (id, _, _) = resolve_workspace_context().await?;
    if id == "tnk-config" {
        return Err(color_eyre::eyre::eyre!(
            "sandbox stop is only available inside a project directory"
        ));
    }
...
fn get_specs_rev() -> String {
    ...
    if let Ok(rev) = std::fs::read_to_string(specs_dir.join(".specs_rev")) {
        let rev = rev.trim();
        if !rev.is_empty() {
